...
const Sidebar = ({ sidebarOpen, setSidebarOpen, onNavigationClick, onSignOut, selectedBranch, selectedSubBranch, navigation }) => {
    const [expandedBranches, setExpandedBranches] = useState(['Business', 'Customers']);
                                        ...
                                        return (
                                            <li key={subBranch.name}>
                                                <button
                                                    ...
                                                    className={`flex items-center justify-between w-full px-3 py-2 text-sm font-medium rounded-lg transition-colors ${isActive ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}`}
                                                >
                                                    <div className="flex items-center space-x-3">
                                                ...
                                                {isBranchExpanded(subBranch.name) && subBranch.subBranches && (
                                                    <ul className="ml-6 space-y-1 mt-1">
                                                        {subBranch.subBranches.map((item) => (
                                                            <li key={item}>
                                                                <button
                                                                    onClick={() => onNavigationClick(subBranch.name, item)}
                                                                    className={`flex items-center w-full px-3 py-2 text-sm font-medium rounded-lg transition-colors ${selectedSubBranch === item ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}`}
                                                                >
                                                                    {item}
                                                                </button>
                                                            </li>
